...
**The `?v=` convention:** every HTML reference to `css/style.css` and `js/main.js` carries a `?v=YYYYMMDD` query (currently `?v=20260617`). With the `no-cache` Caddy policy now live, **bumping it is no longer required** — ETag revalidation already serves fresh CSS/JS on the next request, so a plain edit + deploy is enough. The `?v=` was the one-time flush for browsers that had already cached the old `immutable` copy (only a new URL or a hard refresh can dislodge an already-`immutable` asset; server-header changes can't reach it). Keep the query string in place as harmless belt-and-suspenders; bump it only if CSS/JS are ever moved back onto a long `max-age`, where it would become load-bearing again. One-liner to bump if ever needed:
```bash
